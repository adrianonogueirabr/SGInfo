...
include "conexao.php";
	...
	//TITULO A VISTA
			...
			$sql = $con->prepare("INSERT INTO TBL_TITULORECEBER_TR VALUES (NULL,$empresa_usuario,'$id_usuario','$idCliente', now(), '$novaDataPag','CM$idContrato','$valorCM',0,0,0,0,0,0,'$valorCM',NULL,'AB')"); 
			if(! $sql->execute() )
			{
			...
			$sql = $con->prepare("UPDATE TBL_CLIENTE_CLI SET TXT_TITULOABERTO_CLI='S' WHERE NUM_ID_CLI = $idCliente"); 
			if(! $sql->execute() )
			{
			  die('Houve um erro no processamento da transação: ' . mysqli_error());
			}
